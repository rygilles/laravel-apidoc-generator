...
namespace Rygilles\ApiDoc\Commands;
...
use Rygilles\ApiDoc\Generators\DingoGenerator;
...
class GenerateDocumentation extends Command
{
	...
	public function handle()
	{
		if ($this->option('router') === 'laravel') {
			...
			// Rygilles : Set parent command for extra console logs
	...
	private function setUserToBeImpersonated($actAs)
	{
		if (! empty($actAs)) {
			if (version_compare($this->laravel->version(), '5.2.0', '<')) {
				...
				// Rygilles : removed (int) explicit conversion for UUID support
